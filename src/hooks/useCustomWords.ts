...
/**
 * Custom user-authored vocabulary, stored per (categoryId/subcategoryId) in localStorage.
 * Also supports overriding existing built-in words.
 */
...
export function useCustomWords(categoryId?: string, subcategoryId?: string) {
  ...
  const getOverride = useCallback((wordId: string) =>
    overrides[wordId] || null, [overrides]);
  ...
  const isCustom = useCallback((id: string) =>
    customWords.some(w => w.id === id), [customWords]);
