...
// FIX: Localized type boundaries and core utility definitions to eliminate backend build leakage.
...
// Dummy/Stubs to fulfill standard API layer routing contracts. Ensure your Client-side bundle has 
// these core engine validation and lookup functions mapped out locally or imported from a client module.
...
declare const getChessLevel: (category: string) => any;
declare const resolveLessonText: (key: string, lang: string) => string;
...
export function ChessLessonView({ category, subcategory, lessonId, onNext }: Props) {
  ...
  const lesson = useMemo(() => {
    ...
    const group = level?.groups.find((g: any) => g.id === subcategory);
    const lessonEntry = group?.lessons.find((l: any) => l.id === lessonId);
    ...
    if (Array.isArray(raw)) {
      ...
      return {
        ...
        randomStars: undefined as number | undefined,
        done: undefined as string | undefined,
        introArrows: undefined as Arrow[] | undefined,
      };
    }
    return null;
