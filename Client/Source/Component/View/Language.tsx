...
// Localized type boundaries to prevent backend cross-contamination
...
export function DictionaryBrowseView({ targetLang, targetLangCode }: { targetLang: SupportedLang; targetLangCode: string }) {
  ...
  const { data: corpus } = useQuery({
    queryKey: ["languageCorpusBackend", targetLang],
    queryFn: () => fetchLanguageCorpusFromBackend(targetLang),
    staleTime: 1000 * 60 * 30,
  });

  const { data: labelCorpus } = useQuery({
    queryKey: ["languageCorpusBackend", i18nLang],
    queryFn: () => fetchLanguageCorpusFromBackend(i18nLang),
    staleTime: 1000 * 60 * 30,
    enabled: i18nLang !== targetLang,
  });

  const labels = i18nLang === targetLang ? corpus?.labels : labelCorpus?.labels;

  const builtInCategoryIds = useMemo(
    () => Object.keys(corpus?.vocabularyGrammar?.["Vocabulary"] ?? {}),
    [corpus],
  );

  const categories = useMemo(() => {
    const builtIns = builtInCategoryIds.map(catId => ({
      id: catId,
      label: labels?.[catId] ?? catId,
      count: Object.keys(corpus?.vocabularyGrammar?.["Vocabulary"]?.[catId] ?? {}).length,
      custom: false,
    }));
  ...
  }, [builtInCategoryIds, labels, corpus, collections, uiLang]);
