// FIX: Localized type boundary to prevent backend cross-contamination
...
export function urlToBrowsePath(pathname: string): string[] | null {
  ...
  if (root === "language") {
    ...
    if (branch === "dictionary") {
      ...
      // FIX: Clean structural client parsing. 
      // If content length is 1, it's a structural root category.
      if (contentSegs.length === 1) {
        return ["language", langCode, "dictionary", subKey, contentSegs[0]];
      }
      // If content length is 2, it maps directly to a clean subcategory layout.
      if (contentSegs.length === 2) {
        return ["language", langCode, "dictionary", subKey, contentSegs[0], contentSegs[1]];
      }
      // If content length is 3, it represents a deep entry word definition.
      if (contentSegs.length === 3) {
        return ["language", langCode, "dictionary", subKey, contentSegs[0], contentSegs[1], contentSegs[2]];
      }
